...
**Kurzbefehle** (seit v13): `/n` = `/neu`, `/p` = `/projekte`, `/m` = `/modus`, `/ml` = `/modell`, `/ss` = `/sessions`, `/w` = `/wechsel`, `/st` = `/status`, `/u` = `/usage`, `/c` = `/clear`, `/e` = `/ende`, `/f` = `/fortsetzen`, `/v` = `/verwerfen`, `/rc` = `/remote-control`, `/h` und `/s` = `/start`. Die Expansion passiert an einer einzigen Stelle direkt nach der Groß-/Kleinschreibungs-Normalisierung — unten in der Befehlskette kommt immer die Langform an. Eigene Kürzel: Eintrag in `ALIAS` ergänzen, der Hilfetext zieht automatisch nach.
...
- **v15.1** (2026-09-21): `/s` zeigt die Hilfe (wie `/h`) — bewusst auf `/start` gelegt statt auf `/status` oder `/sessions`, weil `/s` intuitiv getippt wird und ein Vertipper besser in der Hilfe landet als in einer ungewollten Ausgabe. Außerdem liest die Versions-Selbstprüfung den Changelog-Kopf jetzt bis zur ersten `import`-Zeile statt bis Zeile 40 — der Kopf war darüber hinausgewachsen und die Prüfung meldete eine Drift, die keine war
